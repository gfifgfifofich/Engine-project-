...
Explodion NewExplodion(glm::vec2 position, float r, float dmg, float lifetime)
{
	Explodion e;
...
void SpawnExplodion(glm::vec2 position, float r, float dmg, float lifetime)
{
	ExplodionArray.push_back(NewExplodion(position, r, dmg, lifetime));
}
...
void ProcessExplodions(float dt)
{
	...
	for (int i = 0; i < ExplodionArray.size(); i++)
	{
		...
		DrawLight(ExplodionArray[i].DS.body.position, glm::vec2(ExplodionArray[i].DS.body.r * stage * 15.0f), glm::vec4(Color.x * stage*2.0f, Color.y * stage * 2.0f, Color.z* stage * 2.0f, 0.0f));
