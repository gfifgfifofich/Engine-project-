...
std::vector<std::pair<std::string,int>> WaveContent;// pair<name,cost>

void addShipToWave(std::string s, int cost )
{
	WaveContent.push_back(std::pair<std::string, int>(s, cost));
}

bool Wave = false;
int WaveValue = 0;
float WaveFreq = 1.0f;//  1 craft/s
float WaveTimer = 0;
...
void WaveProcess(float dt)
{
	if (WaveValue <= 0)
		Wave = false;
	WaveTimer += dt;
	if (WaveTimer >= WaveFreq)
	{
		WaveTimer = 0;

		glm::vec2 pos = Rotate(glm::vec2(1000.0f, 0.0f), rand() % 100000 / 100000.0f * pi * 2.0f);
		...
		for (int i = 0; i < 100; i++)
		{
			if (id >= 0)
				break;
			id = rand() % WaveContent.size();
			if (WaveContent[id].second > WaveValue)
				id = -1;
		}
		if (id >= 0)
		{
			SpawnAiShip(pos, WaveContent[id].first);

			WaveValue -= WaveContent[id].second;
		}
...
void On_Create()
{
	...
	ships.push_back("Radiat.sav");
	ships.push_back("Save0.sav");
	ships.push_back("Save0Laser.sav");
	ships.push_back("BigBoy.sav");
	ships.push_back("Spinner.sav");
...
void On_Update()
{
	...
	if (!OpenMenu && !MainMenu)
	{
		...
		UI_buttonOnlyON(&Wave, "Wave", { 0.4f * WIDTH, 0.47f * HEIGHT }, UISize, TextSize, glm::vec4(5.0f, 0.0f, 0.0f, 1.0f), glm::vec4(0.5f), 1200);
